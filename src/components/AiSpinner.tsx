...
const AiSpinner: React.FC<AiSpinnerProps> = ({ 
  ...
  return (
    <div className={`relative ${sizeClasses[size]} p-[3px] rounded-full ai-gradient-border ${className}`}>
      <div className="w-full h-full bg-zinc-50 dark:bg-zinc-950 rounded-full flex items-center justify-center">
        <span className="animate-pulse">
          <StarsIcon className="text-yellow-500 dark:text-yellow-300 h-5" />
        </span>
      </div>
    </div>
  );
